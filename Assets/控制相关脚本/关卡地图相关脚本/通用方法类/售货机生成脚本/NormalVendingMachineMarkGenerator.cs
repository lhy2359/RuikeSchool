...
/// <summary>
/// 正常自动售货机生成器（2行1列纵向，当前格子和上方格子都存在才有效）
/// </summary>
public class NormalVendingMachineMarkGenerator : BaseMarkPointGenerator
{
    public NormalVendingMachineMarkGenerator(Scene targetScene, string tilemapName, GameObject vendingPrefab)
        : base(targetScene, tilemapName, vendingPrefab) { }
    ...
    protected override void RecordToLevelSystem(List<Vector3> positions)
    {
        foreach (var pos in positions)
        {
            PlainLevelSystem.Instance.AddNormalVendingMachinePosition(pos);
        }
    }
}